import { c as create_ssr_component, o as onDestroy, d as add_attribute, v as validate_component } from "./index2.js";
import "frappe-charts";
import { d as donationService } from "./donation-service.js";
import { l as latestDonation } from "./stores.js";
...
const Base$1 = Base;
const DonationsByCandidate = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  latestDonation.subscribe(async (donation) => {
    if (donation) {
      await refreshChart();
    }
  });
  let totalByCandidate = { labels: [], datasets: [{ values: [] }] };
    ...
    candidates.forEach((candidate, i) => {
      ...
      donationList.forEach((donation) => {
        if (donation.candidate._id == candidate._id) {
          totalByCandidate.datasets[0].values[i]++;
        }
      });
    });
  }
  ...
  return `<h1 class="${"title is-4"}">By Candidate</h1>
${validate_component(Base$1, "Chart").$$render($$result, { data: totalByCandidate, type: "bar" }, {}, {})}`;
});
export {
  Base$1 as B,
  DonationsByCandidate as D
};
